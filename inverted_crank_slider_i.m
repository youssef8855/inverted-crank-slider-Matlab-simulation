...
l1=0.6; l2=0.3; l3=0.2; l4=0.8; omg2=4; tho1=0; tho2=60*pi/180; tho3 =-52.64*pi/180; tho4=-52.64*pi/180; b=2; 
dt=0.05;% Step size
t_end=2; t_start=0; 
...
q_num=[0 0 0 0.5*l2*cos(tho2) 0.5*l2*sin(tho2) tho2 l2*cos(tho2)+l3*sin(-tho3) l2*sin(tho2)+l3*cos(-tho3) tho3 l1-0.5*l4*cos(tho4) 0.5*l4*sin(-tho4) tho4 ]qd_num=zeros(1,12);
qdd_num=zeros(1,12);
...
C=[ q1;
    ...
    q4-0.5*l2*cos(q6);
    q5-0.5*l2*sin(q6);
    q4+0.5*l2*cos(q6)-q7-l3*sin(q9);
    q5+0.5*l2*sin(q6)-q8+l3*cos(q9);
    q9-q12;
   (q8-q11+b*sin(q12))*cos(q9)-(q7-q10+b*cos(q12))*sin(q9);
...
%required(3)
figure(3);
PX= Res_mat(:,8)+ 0.1*cos(Res_mat(:,10))-0.15*sin(Res_mat (:,10));
PY= Res_mat(:,9)+0.1*sin(Res_mat(:,10))+0.15*cos(Res_mat(:,10));
plot(PX,PY);
